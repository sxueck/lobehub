...
- 类型：可选
- 描述：控制用户是否可以访问助手消息频道配置页面 `/agent/:id/channel`。
- 默认值：`1`
- 示例：`0`

将该变量设置为 `0` 后，会隐藏助手侧边栏里的消息频道入口，并阻止用户直接访问该配置路由。
...
- 类型：可选
- 描述：指定应用管理员用户 ID 列表，多个用户 ID 用英文逗号分隔。
- 默认值：`-`
- 示例：`user_abc123,user_def456`

命中的用户会在设置页中看到额外的管理设置。当前管理员能力支持查看系统中的全部注册用户。
