...
### `ENABLE_MESSAGE_CHANNELS`

- Type: Optional
- Description: Controls whether users can access the agent message channel configuration page at `/agent/:id/channel`.
- Default: `1`
- Example: `0`

Set this variable to `0` to hide the message channel entry in the agent sidebar and block direct access to the configuration route.
...
- Type: Optional
- Description: Comma-separated list of user IDs that should be treated as application administrators.
- Default: `-`
- Example: `user_abc123,user_def456`

Users listed here will see an additional admin settings page. The current admin capability allows viewing all registered users in the system.
